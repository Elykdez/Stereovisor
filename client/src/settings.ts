...
export interface AppSettings {
  ...
  service: {
    showConsole: boolean;
    // Blank means the Vite proxy in development and bundled loopback service
    // otherwise. An explicit origin selects an independently hosted server.
    origin: string;
    accessToken: string;
  };
  camera: {
...
/**
 * Persisted service origins are untrusted input. Keep only a parseable http(s)
 * origin; anything else degrades to "" so the client uses its built-in default.
 */
function originSetting(value: unknown): string {
...
export function sanitizeAppSettings(value: unknown): AppSettings {
  ...
  return {
    ...
    service: {
      ...
      origin: originSetting(service.origin),
