...
if [ -d "$runtime_root" ] && ! "$runtime_root/bin/python3" -c 'import platform, sys; sys.exit(0 if platform.system() == "Darwin" and platform.machine() == "arm64" else 1)' >/dev/null 2>&1; then
  echo "Replacing an incompatible .python-runtime with the Apple Silicon runtime."
  rm -rf "$runtime_root"
fi
