...
def publish_bootstrap_status(
    ...
    # The staging name carries the pid: the launcher publishes status from
    # PowerShell at the same time, and a shared temp name lets two writers
    # collide on Windows even when each one is individually correct.
    temporary = status_path.with_name(f"{status_path.name}.{os.getpid()}.tmp")
    try:
        temporary.write_text("\n".join(lines) + "\n", encoding="utf-8")
        # os.replace fails on Windows while any process holds the destination
        # open, and /api/health reads this file continuously while the startup
        # gate is up. Retry briefly, then give up quietly: status is diagnostic
        # only, and a missed update must not stop model preparation.
        for _ in range(20):
            try:
        ...
        try:
            ...
            # Cleanup must not escape either. An unremovable staging file is
            # what turned a transient sharing violation into a failed launch.
            pass

from service.src.config import MODEL_ROOT  # noqa: E402
from service.src.ai_models import (  # noqa: E402
    DA3_ID,
    DA3_PATH,
    GROUNDING_DINO_ID,
    GROUNDING_DINO_PATH,
...
from service.src.pipeline import (  # noqa: E402
