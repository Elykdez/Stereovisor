...
# Durable job state lives beside the projects it produces. Project ids are
# 32 hex characters, so a dotted filename cannot collide with one.
JOB_DB_PATH = Path(os.environ.get("STEREOVISOR_JOB_DB", PROJECT_ROOT / ".jobs.db"))
# Long enough for a renderer to collect a result after a reload, short enough
# that a long-lived install does not accumulate finished work forever.
JOB_RETENTION_SECONDS = 3600.0


def _port_from_environment(name: str, fallback: int) -> int:
    """Read a TCP port, ignoring blank, malformed, or out-of-range values."""
    try:
        port = int(os.environ.get(name, "").strip())
    except ValueError:
        return fallback
...
def _origins_from_environment() -> list[str]:
    # The renderer is a cross-origin dev host, so the allowlist stays explicit
    # rather than being widened to "*" once the bind address is configurable.
    configured = os.environ.get("STEREOVISOR_ALLOWED_ORIGINS", "").strip()
    if configured:
        return [origin.strip() for origin in configured.split(",") if origin.strip()]
    return ["http://127.0.0.1:5173", "http://localhost:5173", "null", "file://"]


# Network boundary. The default keeps the service on loopback beside the app.
# Non-loopback binds require the bearer token enforced by every HTTP and
# WebSocket route in service.src.app.
SERVICE_HOST = os.environ.get("STEREOVISOR_SERVICE_HOST", "").strip() or "127.0.0.1"
SERVICE_PORT = _port_from_environment("STEREOVISOR_SERVICE_PORT", 5772)
SERVICE_AUTH_TOKEN = os.environ.get("STEREOVISOR_AUTH_TOKEN", "").strip()
...
def ensure_bind_allowed(host: str | None = None, token: str | None = None) -> None:
    """Refuse to expose project routes beyond this machine without a token."""
    resolved_host = SERVICE_HOST if host is None else host
    resolved_token = SERVICE_AUTH_TOKEN if token is None else token
    if is_loopback_host(resolved_host) or resolved_token:
        return
    raise RuntimeError(
        f"Refusing to bind {resolved_host}: remote access requires authentication. "
        "Set STEREOVISOR_AUTH_TOKEN to a strong shared secret."
    )
...
POWERPAINT_VENDOR = Path(
    os.environ.get(
        ...
        WORKSPACE_ROOT / ".cache" / "vendor" / "PowerPaint",
    )
)
    ...
    try:
        ...
        if lines and lines[0] in {
            ...
            if marker.is_file() or parsed_state in {"ready", "blocked"}:
                ...
                fields = dict(line.split("=", 1) for line in lines[2:] if "=" in line)
                provider = fields.get("provider") or None
                completed = tuple(
                    key
                    for key in (
                        entry.strip()
                        for entry in fields.get("completed", "").split(",")
                    )
...
def snapshot_ready(path: Path, required_files: tuple[str, ...] = ()) -> bool:
    ...
    return (path / ".stereovisor-ready").is_file() and all(
...
def powerpaint_snapshot_ready(path: Path) -> bool:
    ...
    return snapshot_ready(path, required_files) and any(
        (path / name).is_file() for name in unet_variants
    )
...
def runtime_dependency_status() -> DependencyStatus:
    ...
    missing = [
        label
        for module, label in AI_RUNTIME_MODULES
        if importlib.util.find_spec(module) is None
    ]
    if missing:
        return DependencyStatus(
            available=False,
            detail=f"Missing local AI runtime packages: {', '.join(missing)}",
        )
...
def model_dependency_status(
    ...
    *,
    ...
    if missing:
        return DependencyStatus(
            available=False, detail=f"Missing model assets: {', '.join(missing)}"
        )
...
def ai_dependencies() -> dict[str, DependencyStatus]:
    ...
    return {
        ...
        "inpainting": model_dependency_status(
            "torch", "Big LaMa", (MODEL_ROOT / "big-lama.pt",)
        ),
        "prompting": DependencyStatus(
            available=qwen_ready,
            detail=(
                "Qwen3-VL 2B weights installed"
                if qwen_ready
                else "Optional Qwen3-VL weights are incomplete or not installed"
            ),
        ),
...
def production_available(
    ...
    """Whether every required provider is present.

    A probe walks the model tree and resolves the AI packages, so a caller that
    already holds one passes it in. One health payload used to run that scan
    twice: once for its own report, once through the engine selection below.
    """
    resolved = ai_dependencies() if dependencies is None else dependencies
    return all(resolved[key].available for key in REQUIRED_PROVIDERS)


def active_engine(dependencies: dict[str, DependencyStatus] | None = None) -> str:
