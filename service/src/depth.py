...
def _resize_array(
    array: np.ndarray, size: tuple[int, int], resample: Image.Resampling
) -> np.ndarray:
...
def normalize_depth(depth: np.ndarray, size: tuple[int, int]) -> np.ndarray:
    ...
    if squeezed.ndim != 2:
        raise RuntimeError(
            f"Depth Anything 3 returned an unexpected depth shape: {squeezed.shape}"
        )
...
def estimate_near_map(image: Image.Image) -> tuple[np.ndarray, int]:
    ...
    logger.info(
        "depth inference started: size=%sx%s device=%s",
        image.width,
        image.height,
        device,
    )
...
def _largest_components(
    binary: np.ndarray, minimum_area: int, maximum_area: int
) -> np.ndarray:
    try:
        import cv2
    except ImportError as error:
...
def foreground_depth_plane(
    near_map: np.ndarray, instance_masks: list[np.ndarray]
) -> np.ndarray | None:
    ...
    candidate = cv2.morphologyEx(
        candidate, cv2.MORPH_OPEN, np.ones((3, 3), dtype=np.uint8)
    )
